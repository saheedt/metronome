...
- **Missing-field check was inconsistent and incomplete**: The AI's `streamCsv` used `!record[f]` (truthiness check) and `streamJson` used `!(f in value)` (key existence only). The CSV version would incorrectly skip records with falsy values like `"0"`. The JSON version only checked key existence but would pass through empty strings as valid. Unified both to `!(f in record && record[f] !== '')`, which checks that the key exists and the value is not an empty string. Added tests for both CSV and JSON to verify that records with empty required fields are correctly skipped.
