...
- **CSV missing-field check used truthiness instead of key existence**: The AI's `streamCsv` used `!record[f]` to detect missing fields, which is a truthiness check. This means falsy values like empty strings `""`, `"0"`, or `false` would be incorrectly treated as missing and the record would be skipped. Fixed by changing to `!(f in record)` to check for key existence only, matching the pattern already used in `streamJson`. The AI got the JSON version right but used a different, buggy pattern for CSV.
